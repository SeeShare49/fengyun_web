...
class TianYu
{
    ...
    public function notify_url()
    {
        ...
        if (isset($result)) {
            ...
            Log::write("status:" . $ret_str['status']);
            if ($ret_str['status'] == "0") {
                $cpOrderNumber = $result['game_order'];
    ...
    {
        $xml = "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"no\"?><quicksdk_message><message><is_test>1</is_test><channel>0</channel><channel_uid>fa820cc1ad39a4e99283e9fa555035ec@_()</channel_uid><channel_order></channel_order><game_order>1_429497729920210705174637</game_order><order_no>00020210705174638757160453</order_no><pay_time>2021-07-05 17:46:38</pay_time><amount>10.00</amount><status>0</status><extras_params></extras_params></message></quicksdk_message>";


        Log::write("response return xml:" . $xml);
        $ret_str = $this->xml_to_data($xml);
        var_dump("ret str:" . $ret_str);

        Log::write("status:" . $ret_str['status']);
        if ($ret_str['message']['status'] == "0") {
            $cpOrderNumber = $ret_str['game_order'];
            $orderNumber = $ret_str['order_no'];
